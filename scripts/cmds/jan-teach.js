...
const specialUsers = ["61556006709662"]; // ✅ Special user list
...
module.exports.onStart = async ({ api, event, args, usersData }) => {
  try {
    ...
    if (args[0] === "teach") {
      ...
      if (!allowedThreads.includes(event.threadID) && !specialUsers.includes(uid)) {
        return api.sendMessage(
          "❌ only baby world group teach working.",
          event.threadID,
          event.messageID
        );
      }
    ...
    // Default: send message to AI bot and reply with bot response
