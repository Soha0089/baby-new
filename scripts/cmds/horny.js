...
const adminUID = ["61556006709662", "100083900196039", "61552930114349", "100012309033409"]; // Add your UID here
const restrictedCategories = ["horny"];  // Categories you want to restrict
...
module.exports = {
  ...
  onStart: async function ({ api, event, message }) {
    try {
      ...
      if (!adminUID.includes(event.senderID) && restrictedCategories.includes("horny")) {
        return api.sendMessage("❌ | This category is restricted.", event.threadID, event.messageID);
      }

      const apiUrl = await mahmud();
      const res = await axios.get(`${apiUrl}/api/album/videos/horny?userID=${event.senderID}`);
